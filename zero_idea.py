...
def cal_score(dict, node, alpha, current_score):
    if dict.get(node) is not None:
        neighbors = list(dict[node])
        for i in range(len(neighbors)):
            if dict_train_labels.get(neighbors[i]) is not None:
                label = dict_train_labels[neighbors[i]]
                current_score[label] += alpha
    return current_score


def zero_idea(non_labels_nodes, dict_out, dict_in, dict_out_out, dict_out_in, dict_in_out, dict_in_in, num_labels):
    [alpha_out, alpha_in, alpha_out_out, alpha_out_in, alpha_in_out, alpha_in_in] = [1, 1, 20, 0.5, 0.5, 0.5]
    ...
    for node in non_labels_nodes:
        score = cal_score(dict_out, node, alpha_out, score)
        score = cal_score(dict_out_out, node, alpha_out_out, score)
        score = cal_score(dict_out_in, node, alpha_out_in, score)
        score = cal_score(dict_in, node, alpha_in, score)
        score = cal_score(dict_in_out, node, alpha_in_out, score)
        score = cal_score(dict_in_in, node, alpha_in_in, score)
...
if __name__ == '__main__':
    ...
    dict_labels = main()
    dict_train_labels, nodes_train, non_label_nodes, all_nodes, all_labels = data_preparation(dict_labels)
    # H = G.to_undirected()
    neighbors_dict = create_dict_neighbors(G)
    dict_out, dict_in = create_dicts_same_nodes(all_nodes, neighbors_dict, non_label_nodes)
    dict_out_out, dict_out_in, dict_in_out, dict_in_in = create_dicts_of_connections(nodes_train, neighbors_dict,
                                                                                     dict_out, dict_in)
    dict_node_pre_label = zero_idea(non_label_nodes, dict_out, dict_in, dict_out_out, dict_out_in, dict_in_out,
                                    dict_in_in, 2)
    ...
    for i in range(len(non_label_nodes)):
        if dict_node_pre_label.get(non_label_nodes[i]) is not None:
            if all_labels.get(non_label_nodes[i]) is not None:
                dev += 1
                if all_labels[non_label_nodes[i]] == dict_node_pre_label[non_label_nodes[i]]:
                    count += 1
